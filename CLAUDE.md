...
**Discounts (`Discounts` object in `discounts.js`):** Gate-admission discount rules, now housed as a tab inside the Admission panel (alongside Pricing and Membership) rather than a standalone nav item. Each rule stores `{ schedule, discountType, demoKey, bracketName, targetLabel, roundCreated, moneyLost }`. **Schedule** replaces the old day-picker + frequency pair: `weekends` (period 1, dayMult 2/7), `weekdays` (period 1, dayMult 5/7), `monthly` (period 4, dayMult 1/7). **Who qualifies** is a single flat selector restricted to ID-verifiable targets: Everyone, the five Age brackets, and named Visitor Status brackets (Disabled, Veteran, Disabled Veteran) — unverifiable demographics (income, household, area, employment, distance) are excluded. `isActiveThisRound(rule)` fires when `(round − roundCreated) % period === 0`. `calcGateCost(weeklyAttendance, gatePrice)` estimates the affected-visitor slice via chance × favor weighting (or fraction = 1 for Everyone) and deducts `affected × gatePrice × costFraction × dayMult`. For named-bracket rules, it also calls `Population.observeDiscount(catKey, bracketIdx, affected)` — visitors presenting ID are direct demographic observations that increment `Population.confidence`. It also pushes a randomly chosen quote from `DISCOUNT_QUOTES` onto `Finance.feedback` (weighted by `affectedCount`) so the quote surfaces as a visitor speech bubble in the animation system. `getFavorBoost(catKey, bracketName)` returns the additive favor boost injected into `calcDemandMultiplier()` from all active rules targeting that bracket (Everyone rules boost all brackets).

**Admission panel (`#panel-financial`):** Three tabs — **Pricing** (gate/concession price controls, built by `_buildPricingTab()`), **Membership** (calls `Membership.buildSection()`), **Discounts** (calls `Discounts.buildPanel()`). Active tab persists across panel close/reopen via `_financialActiveTab` in `hud.js`. Tab HTML is static in `index.html`; content is JS-rendered into `#fin-pricing-view`, `#fin-membership-view`, `#fin-discounts-view`. The `#membership-section` and `#discounts-panel-body` element IDs are preserved inside those views so the respective build functions need no change.
...
**Educational forms (`pl-statement.js`, `balance-sheet.js`, `budget.js`, `tax-form.js`):** Periodic exercises that interrupt gameplay to test financial literacy. Each form is a plain object in its own file with `pending`, `init()`, and `show()`. `hud.js` sets the pending flag(s) in `advanceRound()` and chains `show()` from `hideRoundSummary()`. Chain order: Budget tentative → P&L → Balance Sheet → Budget revised → Tax Return. The P&L statement (`PLStatement`) fires every 13 rounds; the balance sheet (`BalanceSheet`) fires every 52 rounds and chains from `PLStatement.hide()`. The budget (`Budget`) is a two-phase manual-entry exercise: Phase 1 (tentative, `round % 13 === 11`) fires before the P&L — student forecasts blind with only prior quarter's revised budget as reference; Phase 2 (revised, `round % 13 === 1 && round > 1`) fires after the P&L — shows tentative budget and last quarter actuals side by side. `Budget._tentative[qNum]` and `Budget._revised[qNum]` store submissions keyed by game-quarter index. The tax return (`TaxForm`) fires independently on week 4 of each year starting year 2 (`round % 52 === 4 && round > 52`), covering the prior full year; students classify items as Income or Deductions while a live progressive-bracket panel (15% / 25% / 34%) shows the running tax estimate. On submission, `TaxForm.taxOwed` and `TaxForm.taxDueRound` (week 15 = `round + 11`) are set; a countdown pill appears in the achievement bar and the tax is deducted from cash on the due round, recording `taxExpense` in `History` so it appears in the P&L and Cash Flow forms. See README.md for full pattern documentation.

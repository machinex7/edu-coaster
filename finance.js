...
const Finance = {
  ...
  processRound() {
    ...
    const parking = Parking.calcParkingResult(dailyDemand);
    Parking.parkingSpendingMultiplier = parking.spendingMultiplier;
    Parking.altTransportVisitors      = parking.altTransportVisitors;
    Parking.busRiders                 = parking.busRiders;
    ...
    if (Research.completed.has(RESEARCH_ID.PARKING_FEES) && parking.spendingMultiplier >= 1) {
      Parking.knownFreeZone = Math.max(Parking.knownFreeZone ?? 0, Parking.parkingPrice);
    }
    ...
    const memberParkingRevenue = Research.completed.has(RESEARCH_ID.PARKING_FEES)
      ? Membership.paidParkingVehiclesThisRound * Parking.parkingPrice : 0;
    ...
    const freeParkingLoss       = Research.completed.has(RESEARCH_ID.PARKING_FEES)
      ? Math.round(Membership.freeParkingVisitsThisRound * Parking.parkingPrice) : 0;
    ...
    const busCost = (Parking.busEnabled && Research.completed.has(RESEARCH_ID.BUS_SERVICE))
      ? Parking.BUS_WEEKLY_COST : 0;
    money -= busCost;
    ...
    const parkingAmenityCosts      = Parking.roundParkingAmenityCosts;
    Parking.roundParkingAmenityCosts = 0;
